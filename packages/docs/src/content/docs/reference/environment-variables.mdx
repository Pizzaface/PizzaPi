...
| `PIZZAPI_ROUTES_FILE` | Path to a JSON file declaring read-only trigger Routes, synced into the route table at server startup. Shape: `{ "routes": [{ "eventType", "target", "deliverAs", ... }] }` — same fields as `POST /api/routes`, plus `"origin": "config"` and an optional `"ownerUserId"`. Without `ownerUserId` a config route is operator-level and matches every user's events; with it, only that user's. Removing a route from the file removes it from the table on next sync. | — |
...
| `PIZZAPI_CADDY_IMAGE` | `pizza web` only. Overrides the Caddy image. DNS-01 needs a build containing the provider module: `cloudflare` resolves to a known prebuilt image, any other provider requires an [xcaddy](https://github.com/caddyserver/xcaddy) build set here. Persisted to the web config. | `caddy:2-alpine`, or the prebuilt cloudflare image |
...
Self-hosted [ntfy](https://ntfy.sh) delivers background push to the Android
...
Push runs. See [Mobile push](../deployment/mobile-push.mdx) for the full
