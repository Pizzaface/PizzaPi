...
| `PIZZAPI_CADDY_DNS_PROVIDER` | `pizza web` only. [caddy-dns](https://github.com/caddy-dns) module name (e.g. `cloudflare`) used to solve the DNS-01 challenge for a publicly trusted wildcard certificate. Unset → Caddy's internal CA, which requires no DNS credentials but must be trusted once per device. | `unset` |
| `PIZZAPI_CADDY_DNS_TOKEN` | `pizza web` only. API token for `PIZZAPI_CADDY_DNS_PROVIDER`, passed to the container and referenced from the Caddyfile as `{env.PIZZAPI_CADDY_DNS_TOKEN}` (never written into the file). Required whenever a DNS provider is set. | `unset` |
| `PIZZAPI_CADDY_IMAGE` | `pizza web` only. Overrides the Caddy image. DNS-01 needs a build containing the provider module: `cloudflare` resolves to a known prebuilt image, any other provider requires an [xcaddy](https://github.com/caddyserver/xcaddy) build set here. | `caddy:2-alpine`, or the prebuilt cloudflare image |
