...
| `new_session` | Spawn a session (with cwd, prompt, model, skills, and agent config; legacy `hiddenModels` is migration-only) |
...
| `list_models` / `set_hidden_models` / `get_usage` | Runner-owned model catalog, visibility, and usage queries |
...
Services can also declare **sigil types** (`ServiceSigilDef`) for inline references rendered in the UI, and **trigger filters** (`TriggerFilter` / `TriggerFilterMode`) so subscribers can restrict which events they receive. Trigger subscriptions are reconciled between server and runner via `TriggerSubscriptionsSnapshot`, `TriggerSubscriptionDelta`, and the `trigger_subscriptions_applied` ack in the `/runner` namespace.
