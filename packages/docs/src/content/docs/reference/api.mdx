...
Retrieve the model catalog and persisted visibility policy directly from a runner. `models` contains selectable models; `allModels` also includes hidden models so visibility settings can reveal them again.
...
### `PUT /api/runners/{runnerId}/models`

Replace the runner-local hidden model set. The relay authenticates and forwards this request; the runner persists it in `~/.pizzapi/config.json`.
