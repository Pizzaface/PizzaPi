...
## Client and runner communication
...
Use Bun from the repository root. The root `build` script builds `protocol` → `extension-sdk` → `tunnel` → `tools` → `server` → `ui` → `cli`; the docs site builds separately with `bun run build:docs`. Run the test suite with `bun run test` (it includes isolated package checks). The docs build does not require a full application build.

CI runs these checks from [`.github/workflows/ci.yml`](https://github.com/Pizzaface/PizzaPi/blob/main/.github/workflows/ci.yml); workflow concurrency cancels superseded CI runs, while release runs are not canceled. Source of truth: root [`package.json`](https://github.com/Pizzaface/PizzaPi/blob/main/package.json) and docs [`package.json`](https://github.com/Pizzaface/PizzaPi/blob/main/packages/docs/package.json).
