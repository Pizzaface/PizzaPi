...
| `/goal "{condition}" [flags]` | Set a success condition and optional budget for the session. Flags: `--max-turns`, `--max-tokens`, `--max-cost`, `--evaluator keyword|llm`, `--keyword {word}`. The session stops automatically when the goal is met or a budget is exhausted. |
