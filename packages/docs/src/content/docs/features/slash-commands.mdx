...
| `/goal "{condition}" [flags]` | Set a success condition and optional budget for the session. An evaluator checks the condition each time the agent finishes a run and auto-continues while it is unmet; when the goal is met or a budget is exhausted the goal deactivates and control returns to you (the session is never terminated). Flags: `--max-turns` (counts completed agent runs, not LLM round-trips), `--max-tokens`, `--max-cost`, `--every N`, `--min-turns N`. |
