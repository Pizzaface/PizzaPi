...
| `/goal "{condition}" [flags]` | Set a success condition and optional budget. An evaluator checks it each time the agent finishes a run and keeps the agent working while it's unmet. Flags: `--max-turns`, `--max-tokens`, `--max-cost`, `--every N`, `--min-turns N`, `--evaluator`, `--keyword`. See [Session Goals](/features/goals/). |
