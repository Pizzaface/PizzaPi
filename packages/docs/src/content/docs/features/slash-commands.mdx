...
| `/goal "{condition}" [flags]` | Set a success condition and optional budget for the session. Flags: `--max-turns`, `--max-tokens`, `--max-cost`, `--evaluator keyword|llm`, `--keyword {word}`. |
