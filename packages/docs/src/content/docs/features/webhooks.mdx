...
| `X-Webhook-Timestamp` | Yes | ISO 8601 / RFC 3339 timestamp (e.g. `2025-01-15T10:30:00Z`). Must be no more than **30 seconds in the future** or **5 minutes in the past** of server time. |
| `X-Webhook-Nonce` | Yes | Unique string per delivery (e.g. random hex). Redis-backed replay protection (in-memory fallback if Redis is unavailable); multi-node deployments sharing Redis are covered automatically. |
...
**Success (event delivered):** the response includes `ok`, `eventId`, `sessionIds`, and `spawnedSessions`. A webhook may deliver to one or more configured routes; this is not a single `triggerId`/`sessionId` contract.
...
**Filtered (event type did not match the configured filter):**

```json
{ "ok": true, "filtered": true }
```

**Errors:** invalid signatures return `401`; replayed enhanced-mode nonces return `409`; invalid JSON returns `400`; missing/disabled webhooks return `404`; an unassigned runner without a route returns `500`; unavailable or failed delivery returns `503`. Exact error messages are generated by the current server handler.
...
Click the trash icon to open a confirmation dialog, then click **Delete webhook** to confirm. Deletion is permanent — the webhook ID and secret are gone forever.
...
A webhook with a `runnerId` uses its config-backed spawn route. If no runner is assigned, it can still publish through a matching user-owned route; if there is no route configured for the webhook event, the fire request returns `500`.
...
|---------|-------------|
| `401 Missing/invalid X-Webhook-Signature` | Secret mismatch, or the body was modified between signing and sending (e.g., re-serialized by a proxy). Sign the **exact** bytes you send. In enhanced mode the signed string is `${timestamp}.${nonce}.${rawBody}`. |
| `401 Timestamp too old or future` | The timestamp in `X-Webhook-Timestamp` is more than 30s in the future or more than 5 minutes in the past. Check system clock synchronization. |
...
| `500` | No runner is assigned and no matching route is configured for this webhook event. |
| `503` | No route delivered the event (for example, the runner is offline or rejected the spawn); retry after checking runner connectivity. |
