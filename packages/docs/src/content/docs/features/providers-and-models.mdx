...
<Aside type="tip">
  This page covers **AI model backends** (API keys, custom models,
  authentication) only.

  The separate "Extension Providers" plugin system has been removed. To inject
  context or hook session lifecycle events, write a plain
  [pi extension](/PizzaPi/features/pi-packages/); to integrate an external
  service that owns a long-lived connection, declare a
  [runner service](/PizzaPi/customization/runner-services/).
</Aside>
