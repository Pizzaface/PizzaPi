...
| LAN / tailnet | `<ip-with-dashes>.sslip.io` | e.g. `100-64-1-2.sslip.io` — public wildcard DNS to any IP, no setup |
...
For HTTPS on a LAN/tailnet, put Caddy in front with its internal CA via a
`compose.override.yml` next to the generated compose file:
...
The relay only reads the `Host` header, so any TLS-terminating proxy works.
Trust Caddy's root CA once per device. When `PIZZAPI_TUNNEL_DOMAIN` is unset,
tunnels fall back to the path-prefix schemes above.
