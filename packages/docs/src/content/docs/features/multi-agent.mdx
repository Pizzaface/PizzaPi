...
| `effort` | `string` | | Model default | Reasoning effort: `off`, `minimal`, `low`, `medium`, `high`, `xhigh`, or `max`. Unsupported levels are clamped by the model. |
...
Proactively send a message to a linked child session. Unlike `respond_to_trigger` (which responds to a child's request), `tell_child` always delivers a steering message: an active child receives it at the next agent boundary rather than after finishing its turn. An idle child starts a new turn.
...
| **Model selection** | Via agent definition `model` field or `model` parameter | Via `model` parameter |
| **Reasoning effort** | Via `effort` parameter (also per task) | Via `effort` parameter |
...
```
// 1. Spawn workers
spawn_session({ prompt: "Review packages/server/ for security issues", effort: "high" })
// → sessionId: "worker-1"
