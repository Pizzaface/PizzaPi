...
- No manual polling needed — completion triggers steer into the parent automatically
...
Fired just before `session_complete` (on the wire it is emitted first, as an early signal) when a child hits a **usage-limit / quota** error. This lets the parent react — retry with a different model, skip the task, or escalate to the user. Non-quota provider errors do not fire a separate `session_error`; they surface only as `exitReason: "error"` on `session_complete`. Check the `exitReason` field to distinguish error exits from successful completions.
...
|---|---|---|
| **Communication** | `single` / `parallel`: automatic **steer** completion trigger; `chain`: automatic follow-up result | Triggers + `tell_child` |
| **Blocking** | No — parent continues; completion is delivered automatically | No — parent continues; completion is delivered automatically |
| **Context** | `single` / `parallel`: fully independent child process; `chain`: isolated in-process session | Fully independent session + process |
| **UI** | `single` / `parallel`: separate child sessions in the web UI | Separate session in web UI |
| **Model selection** | Via agent definition `model` field | Via `model` parameter |
| **Best for** | Named agent definitions | Self-contained prompts, another runner, or long-lived children |
| **Agent definitions** | File-based (`~/.pizzapi/agents/`) | Prompt-based (inline) |
| **Modes** | Single, parallel, chain | One session per call |

<Aside type="tip">
  **Do not poll background work.** `subagent` completions arrive automatically. For linked child completions, read the delivered result and acknowledge it only when the child is truly finished. See the [Subagents guide](/PizzaPi/customization/subagents/) for full documentation.
</Aside>
...
Linked child session push notifications are **suppressed by default** — only top-level sessions send push notifications. This applies to both Web Push and the native ntfy delivery path. The legacy per-subscription `suppressChildNotifications` setting is now redundant (child pushes are suppressed unconditionally regardless of it).
