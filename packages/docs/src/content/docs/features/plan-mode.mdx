...
Plan mode puts the agent into a **read-only exploration state** where file writes and destructive commands are blocked at the tool level (and at the OS level when [sandbox](/PizzaPi/security/sandbox/) is active). The agent can read code, search, and analyze — but cannot modify anything until a plan is reviewed and approved.
...
|--------|-------------|-----|
| `/plan` slash command | You (human) | Type `/plan` in the chat input (web UI or TUI) — toggles plan mode on/off |
...
- `subagent` — spawns a subagent process with full write access
- `spawn_session` — spawns an independent child session with full write access

The agent can still use `read`, `bash` (with restrictions), `grep`, `find`, `ls`, and **all MCP tools**. Plan mode does not inspect MCP tools for read/write semantics, so MCP write tools are allowed.
...
<Aside type="tip" title="Sandbox enforcement">
  When the [sandbox](/PizzaPi/security/sandbox/) is active, plan mode activates an **OS-level read-only overlay** via `setReadOnlyOverlay()`. This means the filesystem itself is write-protected — even if a command somehow bypasses the regex checks, the OS blocks the write. Without sandbox, tool-level restrictions apply, but they do not provide equivalent OS enforcement.
</Aside>
...
- Use MCP tools (both read and write MCP tools are allowed)
...
After a plan is approved via the **legacy TUI menu path**, the agent enters **execution mode**. If the plan had steps, PizzaPi tracks progress:
...
<Aside type="note">
  Plans approved through the `plan_mode` tool (the web UI and primary remote flow) exit plan mode and proceed to execution, but they **do not currently track step progress** with `[DONE:n]` tags or the execution checklist.
</Aside>
...
In multi-agent setups where a parent agent spawns child sessions, plan review integrates with the [trigger system](/PizzaPi/customization/subagents/).
...
- `respond_to_trigger(triggerId, "approve", "approve")` — child proceeds with execution
- `respond_to_trigger(triggerId, "cancel", "cancel")` — child stops
- `respond_to_trigger(triggerId, "Your step 2 should also handle middleware", "edit")` — child revises the plan
...
- **State does not persist across resumes** — resuming a session currently starts with plan mode off and the sandbox overlay cleared. Any in-progress execution steps are not restored.
