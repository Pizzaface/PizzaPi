...
// ~/.pizzapi/config.json  (or  .pizzapi/config.json in your project)
{
  ...
  // See the Sandbox guide for full details.
  "sandbox": {
    "enabled": true,
    "mode": "enforce",
    "filesystem": {
...
| `mcpTimeout` | `number` | `30000` | Timeout (ms) for each MCP server's `tools/list` call during startup. Set to `0` to disable. See [Safe Mode](/PizzaPi/guides/safe-mode/). |
| `sandbox` | `object` | *(see below)* | OS-level sandbox config. See [Agent Sandbox](/PizzaPi/guides/sandbox/). |
| `sandbox.enabled` | `boolean` | `true` | Enable sandbox enforcement. |
| `sandbox.mode` | `string` | `"enforce"` | `"enforce"`, `"audit"`, or `"off"`. |
| `sandbox.filesystem.denyRead` | `string[]` | *sensitive dirs* | Paths blocked from reading. |
| `sandbox.filesystem.allowWrite` | `string[]` | `[".", "/tmp"]` | Paths allowed for writing. |
| `sandbox.filesystem.denyWrite` | `string[]` | `[".env", "~/.ssh"]` | Paths blocked from writing. |
| `slowStartupWarning` | `boolean` | `true` | Show warnings when startup takes longer than expected (slow MCP servers, etc.). See [Safe Mode](/PizzaPi/guides/safe-mode/). |
...
| `PIZZAPI_NO_MCP` | — | Set to `1` to skip MCP servers ([Safe Mode](/PizzaPi/guides/safe-mode/)) |
| `PIZZAPI_NO_PLUGINS` | — | Set to `1` to skip plugin loading ([Safe Mode](/PizzaPi/guides/safe-mode/)) |
| `PIZZAPI_NO_HOOKS` | — | Set to `1` to skip hook execution ([Safe Mode](/PizzaPi/guides/safe-mode/)) |
| `PIZZAPI_NO_RELAY` | — | Set to `1` to skip relay connection ([Safe Mode](/PizzaPi/guides/safe-mode/)) |
| `PIZZAPI_SANDBOX` | `sandbox.mode` | Override sandbox mode: `enforce`, `audit`, or `off` ([Sandbox](/PizzaPi/guides/sandbox/)) |
| `PIZZAPI_NO_SANDBOX` | — | Set to `1` to disable sandbox ([Sandbox](/PizzaPi/guides/sandbox/)) |
| `PIZZAPI_SANDBOX_NETWORK` | `sandbox.network.mode` | Override network mode: `denylist`, `allowlist`, or `off` ([Sandbox](/PizzaPi/guides/sandbox/)) |
