...
2. Spawns a `pi` child process with `--mode json --no-session`
...
| **Context** | Isolated child process | Fully independent session |
| **UI** | Inline in parent session | Separate session view |
| **Overhead** | Low (child process) | Higher (relay round-trip, new PTY) |
...
|---------|---------------------|---------------------|
| **Execution** | In-process | Separate `pi` process (isolated context) |
