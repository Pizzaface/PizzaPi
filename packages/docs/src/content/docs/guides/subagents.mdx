...
1. Discovers available agent definitions from `~/.pizzapi/agents/` (user) and `.pizzapi/agents/` (project)
2. Spawns a `pi` child process with `--mode json --no-session`
3. Streams progress updates back to the parent in real-time
...
Agent definitions are markdown files with YAML frontmatter stored in:

- **User scope:** `~/.pizzapi/agents/*.md` — available in all your sessions
- **Project scope:** `.pizzapi/agents/*.md` — repo-specific agents (walk-up search from cwd)
...
| `model` | | Model override (e.g., `claude-haiku-3` for cheaper tasks) |
...
| Scope | Searches |
|-------|----------|
| `"user"` (default) | `~/.pizzapi/agents/` only |
| `"project"` | `.pizzapi/agents/` only |
| `"both"` | Both directories (project overrides user on name conflict) |

### Project agent security

Project-scope agents (`.pizzapi/agents/`) are loaded from the repository and could contain untrusted prompts. When `agentScope` includes project agents:
...
<Aside type="caution">
  Project-scope agents come from the repository's `.pizzapi/agents/` directory. Only use `agentScope: "both"` or `"project"` with repositories you trust.
</Aside>
