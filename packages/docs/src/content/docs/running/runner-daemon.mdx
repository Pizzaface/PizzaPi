...
The **runner daemon** lets you spawn agent sessions on demand — from the web UI or programmatically via the `spawn_session` tool — **without needing a terminal open**.
...
Use a **LaunchAgent** (`~/Library/LaunchAgents/`), not a LaunchDaemon: it runs in your login session and can access your keychain and SSH credentials. Manage it with `launchctl` for your user; see [Apple's launchd documentation](https://support.apple.com/guide/terminal/use-launchd-to-manage-daemons-and-agents-trmlb7a3c61f/mac).
...
This requests a graceful supervisor shutdown, then force-kills if it does not exit within its timeout. The daemon disconnects and releases its lock; session workers receive shutdown handling from the daemon. To restart the runner, stop and start it with `pizzapi runner stop` followed by `pizzapi runner` (or restart its service/container). A daemon restart can re-adopt eligible live sessions; it is not equivalent to ending every session.
...
When sessions are spawned via `spawn_session`, the runner daemon automatically **links them as parent-child**. This enables the conversation trigger system. See the [Subagents and linked sessions](/PizzaPi/customization/subagents/) guide for the full list of trigger types, response actions, and delivery semantics.
