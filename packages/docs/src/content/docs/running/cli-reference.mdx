...
| `update [source\|self\|pi]` | Update installed packages and/or the upstream `pi` package |
| `update --extensions` | Update installed packages only |
| `update --self` | Update the upstream `pi` package only (use `npm install -g @pizzapi/pizza` to update PizzaPi) |
...
<Aside type="note">
  `pizzapi update --self` updates the upstream `pi-coding-agent` package, not the PizzaPi wrapper. To update the wrapper itself, run `npm install -g @pizzapi/pizza`.
</Aside>
