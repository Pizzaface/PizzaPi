...
<Aside type="note">
  **Custom Docker overrides:** The `compose.yml` is regenerated on each run from the template + config. For additional Docker customizations (extra volumes, networks, etc.), create a `compose.override.yml` in the same directory — PizzaPi automatically includes it if present.
</Aside>
...
```
anthropic
  claude-opus-4-5            reasoning • 200,000 ctx • Claude Opus 4.5
  claude-sonnet-4-20250514            200,000 ctx • Claude Sonnet 4

google
  gemini-2.5-pro             reasoning • 1,000,000 ctx • Gemini 2.5 Pro

ollama-cloud
  glm-5.1                    reasoning • 202,752 ctx • GLM 5.1
```
...
- **Global plugins** in `~/.pizzapi/plugins/` and `~/.agents/plugins/` are **always auto-trusted** — same as global skills or extensions. Claude Code marketplace plugins installed via `~/.claude/plugins/installed_plugins.json` are discovered through that manifest, not by scanning `~/.claude/plugins/` directly.
...
Install, update, and manage **pi packages** — bundles of extensions, skills, prompt templates, and themes that extend the agent. These commands inherit the upstream `pi` package manager and store packages in `~/.pizzapi/settings.json` (or `.pizzapi/settings.json` for project-local installs).
...
Delegate tasks to specialized agents. Single and parallel calls start visible linked child sessions; each completion is automatically delivered as a **steer** trigger, so the parent must never poll. Chain calls remain isolated in-process to preserve `{previous}` output substitution and deliver their result automatically as a follow-up.

**Modes:**
- **Single**: `{ agent: "name", task: "..." }` — start one linked child session
- **Parallel**: `{ tasks: [...] }` — start up to 8 linked child sessions (spawn requests are limited to 4 at once)
- **Chain**: `{ chain: [...] }` — sequential in-process execution with `{previous}` output substitution
