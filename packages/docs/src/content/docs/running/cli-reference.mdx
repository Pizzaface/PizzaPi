...
Flags:
  ...
  --image <repo>      Use a prebuilt image repo (e.g. ghcr.io/acme/pizzapi)
  --tag <tag>         Image tag used with --image (default: latest)
...
# Run from a custom GHCR image instead of local source build
pizzapi web --image ghcr.io/acme/pizzapi --tag 0.1.32
...
| `--image <repo>` | Use a prebuilt Docker image repository (for example `ghcr.io/acme/pizzapi`). Persisted to `config.json`. |
| `--tag <tag>` | Image tag used with `--image` (default `latest`). Persisted to `config.json`. |
...
pizzapi web config set image ghcr.io/acme/pizzapi
pizzapi web config set imageTag 0.1.32
```
...
| `image` | *(empty)* | Docker image repo for image mode. Empty means build from source. |
| `imageTag` | `latest` | Docker image tag used with `image`. |
