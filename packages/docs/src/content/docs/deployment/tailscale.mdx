...
- A working PizzaPi deployment (see [Self-Hosting](/PizzaPi/deployment/self-hosting/))
...
2. **Update allowed origins**
   ...
   See the [Self-Hosting guide](/PizzaPi/deployment/self-hosting/) for full details on setting environment variables and override files.
...
Tailscale Serve covers the relay itself, but it cannot cover PizzaPi's
[host-based tunnels](/PizzaPi/deployment/tunnel-tls/), which need a **wildcard**
...
MagicDNS has no wildcard and Tailscale issues certificates only for a node's own
...
at the tailnet IP; see [Tunnel TLS](/PizzaPi/deployment/tunnel-tls/).
