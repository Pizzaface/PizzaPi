...
- **Attachment uploads**: 50 MB hard cap — the server rejects requests larger than 50 MB at the transport layer with a `413 Payload Too Large` before the upload handler runs. The `PIZZAPI_ATTACHMENT_MAX_FILE_SIZE_BYTES` env var can lower this limit further (per-file validation), but **cannot raise it above 50 MB** without a code change to `MAX_ATTACHMENT_BODY_SIZE` in `handler.ts`.
