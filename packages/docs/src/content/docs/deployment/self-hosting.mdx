...
# Use a prebuilt image (e.g. from GitHub Container Registry)
...
If you publish your own server image to GHCR, set it once and it will persist:

```bash
pizzapi web config set image ghcr.io/<org>/<image>
pizzapi web config set imageTag latest
```
