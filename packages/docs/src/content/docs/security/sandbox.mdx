...
PizzaPi can enforce OS-level sandboxing around agent tool execution — bash commands, file reads/writes, and MCP server network access (domain-filtered). This prevents prompt injections or model hallucinations from accessing sensitive files, exfiltrating data, or damaging your system.
