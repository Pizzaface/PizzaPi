...
PizzaPi can enforce OS-level sandboxing around the agent worker's shell and file-tool processes. The policy limits filesystem access and, in `full` mode, outbound network access. It does not constrain browser sessions, standalone service panels, or necessarily every external MCP server's own effects; treat connected integrations as separately trusted capabilities. Sandboxing reduces risk but is not a security boundary for untrusted workloads.
...
`pizza --help` advertises the aliases `enforce`, `audit`, and `off`, which map to `full`, `basic`, and `none`. All six names are accepted.
...
Violations are stored in a ring buffer (100 entries max) inside the worker process. They are **not** streamed to the relay or web UI in real time — they are visible only through the `/sandbox` slash command in an active session.
...
```json
{
  ...
  "rawConfig": { "mode": "basic" }
}
```

| Field | Description |
|-------|-------------|
| `ok` | `true` if the status query succeeded |
| `mode` | Active sandbox mode (`none`, `basic`, `full`) |
| `active` | `true` when a non-`none` mode is configured; this does **not** guarantee enforcement is currently running on the worker |
| `configured` | Same as `active` — a non-`none` mode is configured |
| `platform` | Runner's OS (`darwin`, `linux`, `win32`) |
| `violations` | Always `0` from this REST endpoint (the daemon does not run inside a sandbox) |
| `recentViolations` | Always empty from this REST endpoint |
| `config` | Fully resolved **global** sandbox config (project-local overrides are excluded) |
| `rawConfig` | The raw global sandbox config as stored in `~/.pizzapi/config.json` |

Real-time violations and the fully resolved per-session config are only available via the in-session `/sandbox` slash command.
...
- On the in-session `/sandbox config` path, sending `null` for a key removes it from the config (e.g., clearing stale network rules when downgrading from `full` to `basic`).
- On the REST `PUT /api/runners/{runnerId}/sandbox-config` endpoint, `null` is currently written as a JSON `null` value rather than deleting the key. The effective sandbox value is still cleared, but the key remains in `~/.pizzapi/config.json`.
...
Real-time `sandbox:violation` forwarding to the web UI is **not currently implemented**. Violations are logged in the worker and surfaced through the `/sandbox` slash command in an active session.
