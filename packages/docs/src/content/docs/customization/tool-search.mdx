...
2. **If the total exceeds `tokenThreshold`**, MCP tools are removed from the agent's active tool set (except servers marked `deferLoading: false`). Built-in tools (`read`, `bash`, `edit`, `write`, etc.) are never deferred.
...
When Tool Search is enabled, agents see a single `search_tools` tool with this interface. The tool is registered unconditionally, so it may appear even when Tool Search is disabled or no tools are deferred; calling it in that case reports that tool search is inactive.
...
- **Not active**: if Tool Search is disabled or no tools are deferred, it reports that tool search is inactive.
...
- **`/tool-search status`** — shows how many tools are deferred (with their server names), which are loaded on-demand, and whether Tool Search is active.
- **`/tool-search reset`** — re-evaluates deferral against the threshold. Previously loaded on-demand tools remain loaded.
