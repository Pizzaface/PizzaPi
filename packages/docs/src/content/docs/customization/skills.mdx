...
Skills from global plugins (`~/.pizzapi/plugins/`, `~/.agents/plugins/`) are automatically discovered alongside your regular skills. Plugins under `~/.claude/plugins/` are loaded only through Claude Code's `installed_plugins.json` marketplace manifest, not by scanning the directory. See the [Claude Code Plugins guide](/PizzaPi/customization/claude-plugins/) for details.
...
<Aside type="note">
  Skill names must be lowercase letters, numbers, and hyphens (1–64 characters). The runner rejects underscores, dots, and uppercase letters. The web UI may allow broader names, but a name that doesn't pass the runner validator will fail when saved.
</Aside>
...
Prompt templates are Markdown files exposed as slash commands; they are distinct from skills but use the same project/global customization workflow. Put a `.md` file directly in `<cwd>/.pizzapi/prompts/`, `~/.pizzapi/commands/`, `<cwd>/.pizzapi/commands/`, or `<cwd>/.agents/commands/`. Discovery is non-recursive. A filename such as `review.md` creates `/review`; optional YAML frontmatter can provide a `description` shown in the command picker. The Markdown body is expanded as the prompt, with `$1`, `$2`, `$@`/`$ARGUMENTS` and `${@:N}` argument substitutions. See [slash commands](/PizzaPi/features/slash-commands/) for the picker and built-in commands.
