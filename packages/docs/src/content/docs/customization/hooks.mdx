...
<Aside type="caution" title="Project hooks require explicit trust">
  Project-local hooks (`.pizzapi/config.json`) are **ignored** unless you set `allowProjectHooks: true` in your **global** `~/.pizzapi/config.json`. A one-time warning is emitted when project hooks are present but untrusted. Projects cannot self-authorize — the trust gate lives in global config only.
...
| `BeforeAgentStart` | Before agent turn begins | No (can override system prompt) | No |
...
| `TurnEnd` | After each agent turn completes | No (fire-and-forget) | No |
| `SessionStart` | On session start/reload/new/resume/fork | No (fire-and-forget) | No |
...
| Other non-zero | **Error** — treated as a block (fail-closed for `PreToolUse` and cancelable event hooks only; `PostToolUse`, `BeforeAgentStart`, and fire-and-forget events do not fail-closed) |
...
| `permissionDecision` | PreToolUse | `"deny"` blocks the tool call. `"allow"` and `"ask"` are reserved/future. |
| `updatedInput` | PreToolUse | Object replacing tool input fields (rewrites tool args before execution) |
...
Claude Code plugins can bundle hooks in one or more `*.json` files inside a `hooks/` directory (commonly `hooks/hooks.json`). PizzaPi's plugin adapter automatically discovers these and registers them alongside your config-defined hooks.
...
### 1. Advisory RTK suggestion (PreToolUse)

Injects a suggestion to pipe verbose Bash output through `rtk` to reduce token usage:

```bash
#!/bin/bash
# ~/.pizzapi/hooks/rtk-compress.sh
# PreToolUse hook — suggests RTK compression for verbose Bash commands
INPUT=$(cat)
...
2. **Project hooks** (`.pizzapi/config.json` in a repo) are **ignored by default**.
