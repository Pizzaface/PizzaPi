...
1. **Place a plugin** in one of the global plugin directories:
   ...
   ```bash
   # These are scanned directly:
   ~/.pizzapi/plugins/my-plugin/
   ~/.agents/plugins/my-plugin/

   # Claude Code marketplace plugins are loaded via installed_plugins.json — see below.
   ```
...
<FileTree>
- my-plugin/
  - .claude-plugin/plugin.json *(Claude Code standard location; takes precedence over root plugin.json)*
  - plugin.json *(optional — name, description, version, author, homepage, repository, license, keywords)*
  - commands/
    - review.md
    - deploy.md
  - hooks/
...
If `plugin.json` is missing, the plugin name is inferred from the directory name. PizzaPi also checks `.claude-plugin/plugin.json` first (the Claude Code standard location); if that file exists it takes precedence over a root `plugin.json`.

---

## Commands

Command files are Markdown files in the `commands/` directory. Each file becomes a pi slash command with a flat name derived from its relative path:

```
commands/review.md        →  /review
commands/deploy.md        →  /deploy
commands/a/b/c/deep.md    →  /a/b/c/deep
```

Commands are registered by their file-derived name, not namespaced to the plugin. Two plugins that both ship `commands/review.md` will collide; the last-loaded plugin wins.
...
Hooks are shell commands that run at specific lifecycle events. They're defined in one or more `*.json` files inside the plugin's `hooks/` directory (commonly `hooks/hooks.json`):
...
| `~/.claude/plugins/` | Claude Code marketplace cache — discovered via `installed_plugins.json` + `enabledPlugins` from Claude Code settings |
...
3. **Web UI** — amber trust banner with **Trust & Load** / Skip buttons (when running via the runner daemon)
...
|---------|--------|-------|
| **Commands** | ✅ Adapted | Become pi slash commands |
| **Hooks** | ✅ Adapted | Merged into pi lifecycle events |
| **Skills** | ✅ Adapted | Loaded via the Agent Skills standard |
| **Rules** | ✅ Adapted | Injected into the system prompt |
| **Agents** | ✅ Adapted | Loaded as subagent definitions |
| **MCP servers** (`.mcp.json`) | ⚠️ Shown as warning | Use pi's own MCP integration instead |
| **LSP** (`.lsp.json`) | ⚠️ Shown as warning | Language Server Protocol — Claude-specific |
...
- Plugin names with capability badges (commands, hooks, skills, rules, MCP)
...
# Copy individual plugins you want (symlinked plugin roots are ignored)
cp -r /tmp/claude-plugins/plugins/code-review ~/.pizzapi/plugins/code-review
cp -r /tmp/claude-plugins/plugins/testing ~/.pizzapi/plugins/testing
