...
|-------|----------|-------------|
| `description` | No | Short description shown in the slash command picker. If omitted, the first non-empty line of the body is used (truncated to 60 characters). |
| `argument-hint` | No | Hint shown next to the command name in the **Plugin Commands** picker (e.g. `\u003cbranch\u003e`). |
...
| `${N:-default}` | Positional argument N, with a fallback when missing or empty |
...
- **Commands** — `/new`, `/resume`, `/compact`, etc.
- **Plugin Commands** — commands from plugins and extensions
...
PizzaPi supports the Claude Code `commands/` directory convention alongside its own project-local `prompts/` directories. This means:
