...
- **Add UI panels and widgets** — *capability is validated but not yet wired to the UI*
- **Contribute session metadata** — *capability is validated but not yet consumed by the UI*
...
The loader tries a `new` constructor first for any function export, and falls back to awaiting it as a factory if that throws. In practice this means three common patterns work:
...
`init()` receives a `ProviderInitContext`:

```typescript
interface ProviderInitContext {
  config: Record<string, unknown>;     // Provider-specific config from config.json (includes `enabled`)
  fireTrigger(sessionId, type, payload): Promise<void>;  // ⚠️ currently a no-op stub
  socket: unknown;                     // ⚠️ always `null` today
  publishMetadata(sessionId, metadata): void;  // ⚠️ currently a no-op stub
}
```

The `config` object contains the provider's full entry from `~/.pizzapi/config.json`, including the `enabled` field:
...
**Deduplication:** When a provider emits a contribution with a `dedupeKey` it has already used within the same user prompt, the second is silently skipped. Dedupe is **per-provider** — two different providers using the same key are both emitted. This prevents duplicate context when a provider fires `onBeforeAgentStart` multiple times per prompt (e.g., on retries). The dedupe state resets at the start of each new user prompt.
...
interface SessionStartEvent {
  ...
  model?: { provider: string; id: string; name: string };  // The active model at session start
}
```
...
If a provider throws in `onBeforeAgentStart`, `onTurnEnd`, `onSessionStart`, or `onSessionClose`:
...
4. A successful call resets the counter for `onBeforeAgentStart` and `onTurnEnd` only; `onSessionStart` and `onSessionClose` do not reset it
...
- Check for `[ProviderBridge] Disabling provider` errors in the runner logs
...
- [ProviderBridge source](https://github.com/pizzaface/PizzaPi/blob/main/packages/cli/src/providers/bridge.ts) — how providers are orchestrated
