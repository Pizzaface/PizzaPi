...
Set `effort` to control provider reasoning effort for a subagent. Valid values are `off`, `minimal`, `low`, `medium`, `high`, `xhigh`, and `max`; the selected model clamps levels it does not support. A top-level value applies to every task, while `tasks` and `chain` items can override it individually:

```json
{
  "effort": "low",
  "tasks": [
    { "agent": "researcher", "task": "Scan the API", "effort": "high" },
    { "agent": "reviewer", "task": "Review the scan" }
  ]
}
```
...
<Aside>
  This page includes the agent file format, discovery paths, and tool restrictions below; see [Agent definitions](#agent-definitions).
</Aside>
...
For large fan-out work where intermediate subagent results should stay out of the parent context, a dynamic workflow can orchestrate `agent()` and `pipeline()` calls in a JavaScript script via `run_workflow`. Use a single or parallel subagent for a few tasks, chain mode when each step needs the prior result, and workflows for orchestration with custom logic or many tasks. Workflow scripts run with configured limits; only the final return value is delivered to the parent. See the [workflow implementation](https://github.com/Pizzaface/PizzaPi/tree/main/packages/cli/src/extensions).
...
For large fan-out orchestration that keeps intermediate output out of context, see [When to use workflows](#when-to-use-workflows).
