...
By default, linked child sessions (spawned via `spawn_session`) **do trigger push notifications** just like top-level sessions. Each device subscription has a **Suppress child notifications** toggle — when enabled, that subscription will not receive notifications originating from child sessions. This lets you reduce notification noise when orchestrating many child sessions without silencing all alerts globally.
