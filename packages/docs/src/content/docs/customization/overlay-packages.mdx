...
**Overlay packages and runner services are not two features.** The overlay is the *packaging and trust* layer — how capabilities are declared, installed, scoped, and granted. A [runner service](/PizzaPi/customization/runner-services/) is one *kind of capability* the overlay can carry: the daemon-side process that ships handlers, panels, triggers, and sigils. This page covers the manifest, trust, and SDK; the runner services page covers what a service does once it is loaded.
...
| `agents` | `string[]` | Directories of [agent definitions](/PizzaPi/customization/subagents/#agent-definitions). |
...
Each entry in `services` needs `id`, `label`, and `entry`; `icon`, `panel`, `triggers`, and `sigils` are optional. The full field reference, handler contract, and lifecycle live on the [Runner Services](/PizzaPi/customization/runner-services/#service-declaration) page — this page only covers how the declaration is packaged and trusted.
...
Types are erased at build time; only the three host-detection helpers are runtime code. The `ServiceHandler` contract itself — `init`/`dispose`, `ServiceInitOptions`, and the optional lifecycle hooks — is documented under [ServiceHandler API](/PizzaPi/customization/runner-services/#servicehandler-api).
...
- [Agent Definitions](/PizzaPi/customization/subagents/#agent-definitions) — authoring agents
