...
| `subagent.maxParallelTasks` | `number` | `8` | Max tasks per parallel call and total active or queued subagent work. **Global config only.** |
| `subagent.maxConcurrency` | `number` | `4` | Max active child sessions within one runner-backed parallel call. **Global config only.** |
...
and completion messages. The agent must never poll a backgrounded command with `sleep` or
`bash_output(pid)`: completion is delivered automatically. It may use `bash_output(pid)` only when
new partial output is needed to make a decision; use `kill_shell(pid)` to stop it. Setting
`run_in_background: true` skips the foreground wait entirely.
