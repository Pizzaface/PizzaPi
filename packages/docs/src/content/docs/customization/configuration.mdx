...
| `image` | *(empty)* | Optional Docker image repo. Empty means build from source. |
| `imageTag` | `latest` | Docker image tag used with `image`. |

CLI flags like `--port`, `--origins`, `--image`, and `--tag` also persist to this config file automatically.
