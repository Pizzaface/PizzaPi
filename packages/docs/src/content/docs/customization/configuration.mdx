...
// ~/.pizzapi/config.json  (or  .pizzapi/config.json in your project)
{
  ...
  // transcript and active goal to a model each time the agent finishes a run.
  // When no `evaluatorModel` is set, it uses the cheapest available
  // authenticated text model. Cadence is measured in completed agent runs (one
  // user prompt through to control returning to the user), not LLM round-trips.
  "goal": {
...
| `goal.evaluatorModel` | `string` | cheapest authenticated text model | Model (`provider:modelId` or `modelId`) used by the `/goal` LLM evaluator. |
