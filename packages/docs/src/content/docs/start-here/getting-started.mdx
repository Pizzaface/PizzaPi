...
- **Docker + Docker Compose** — only if you self-host (via `pizza web` or the manual Docker path)
...
2. **Run first-time setup**
   ...
   **Prefer a QR code?** Run `pizzapi setup --scan` to print a terminal QR code. Scan it from an already-authenticated PizzaPi browser to approve the device without typing your password on the new machine.
...
2. **Generate required secrets**
   ...
   The manual `docker/compose.yml` does not include `BETTER_AUTH_SECRET` or VAPID keys. Without them, sessions and push subscriptions are invalidated on every restart. Generate them first:

   ```bash
   export BETTER_AUTH_SECRET=$(openssl rand -hex 32)
   # Optional: provide VAPID keys if you use Web Push (pizza web generates these automatically)
   export VAPID_PUBLIC_KEY=...
   export VAPID_PRIVATE_KEY=...
   export VAPID_SUBJECT=mailto:you@example.com
   ```

   Or use [`pizza web`](#the-easy-way-pizza-web) for a turnkey setup that generates and persists these secrets for you.
...
<CardGrid>
  ...
  <Card title="Todo List" icon="approve-check">
    If the agent creates a task plan, it appears as a live-updating todo card in the session stream — items move from pending → in progress → done.
  </Card>
</CardGrid>
...
| Relay configured | `cat ~/.pizzapi/config.json` | Shows saved API key (run `pizzapi setup` to re-run the wizard) |
