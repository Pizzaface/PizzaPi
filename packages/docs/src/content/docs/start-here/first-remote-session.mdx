...
- **git** — `pizzapi web` clones the PizzaPi repo on first run
...
1. ### Start the relay server
   ...
   This spins up a Docker Compose stack with Redis (relay state / pub-sub), the PizzaPi server (relay API + web UI), a UI sidecar for web assets, and an optional ntfy service for native Android push (configured separately in [Mobile Push](/PizzaPi/deployment/mobile-push/)).
...
4. ### Start a session
   ...
   <Aside type="note">
   Running `pizzapi` with no saved config would auto-launch the setup wizard. Since you just ran `pizzapi setup`, that prompt is skipped.
   </Aside>
