...
<Aside type="tip">
  **Set up by QR code:** On a machine where you’re already logged into the web UI, run `pizzapi setup --scan`. The CLI prints a terminal QR code; scan it from the browser to approve the device without typing your password.
</Aside>
...
| `~/.pizzapi/runner.json` | `pizzapi runner` (first start) | Runner identity, lock/PID tracking, and runner secret (auto-generated) |
| `~/.pizzapi/usage-cache.json` | `pizzapi runner` | Shared provider quota cache (written by daemon, read by workers) |
| `~/.pizzapi/logs/` | manual ([LaunchAgent plist](/PizzaPi/deployment/mac-setup/)) | Log files for runner stdout/stderr |
...
If you previously used the raw [`pi` coding agent](https://pi.dev/), PizzaPi automatically migrates your existing data on first startup. No manual steps are required.

The migration runs in two phases:

1. **Phase 1 — `~/.pi/agent/` → `~/.pizzapi/`**: Moves (merges) sessions, auth, and other data from a legacy `pi` install into the PizzaPi config directory. Individual files are moved out of `~/.pi/agent/`, so the legacy directory is left empty.
...
This overwrites the `apiKey` in `~/.pizzapi/config.json` and prints the WebSocket URL. **Note:** `pizzapi setup` only persists the API key; it does not write `relayUrl`. After setup, manually add the relay URL if it's not the default:
...
See the [Standalone Mode guide](/PizzaPi/running/standalone-mode/) for details.
...
If you set up the runner as a macOS LaunchAgent, see the uninstall steps in the [Runner Daemon guide](/PizzaPi/running/runner-daemon/) and [macOS Setup guide](/PizzaPi/deployment/mac-setup/).

### 5. Remove systemd service (Linux, if installed)

If you created a systemd unit for the runner, see the [Runner Daemon guide](/PizzaPi/running/runner-daemon/) for the stop/disable commands.
