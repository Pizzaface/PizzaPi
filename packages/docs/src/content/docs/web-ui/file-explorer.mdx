...
Toggle the File Explorer with the keyboard shortcut **⌘⇧E** (macOS) or **Ctrl+Shift+E** (Windows/Linux). You can also click the folder icon button in the session viewer header bar (desktop only; on mobile use the keyboard shortcut).
...
The breadcrumb bar at the top shows the current root path (`cwd`). Use the **refresh** button (↻) to re-fetch the directory listing if the filesystem has changed. Next to it, the **Expand all** and **Collapse all** buttons expand every folder up to three levels deep or collapse the whole tree.
...
Markdown (`.md` and `.mdx`) files open in a dedicated Markdown viewer with a **Preview / Raw** toggle. Preview renders the file with LazyStreamdown formatting; Raw shows the plain source text. Like other text files, large Markdown files are read-only and truncated at 512 KB.
...
- **Transparency checkerboard** — a checkerboard is always shown behind rendered images, which conveniently handles images with alpha channels
...
The file search API powers the **@-mention** autocomplete in the message composer. When you type `@` followed by a filename fragment, PizzaPi searches git-tracked files (plus untracked files that are not ignored) under the working directory and presents matching files as suggestions. If the workspace is not a git repository, the search returns an empty list with no error. This uses the same `search-files` endpoint described below.
...
Each entry in `files` includes `name`, `path`, `isDirectory`, `isSymlink`, `relativePath`, and `size` in bytes (reported for both files and directories, though directory sizes are usually not meaningful).
...
| `query` | `string` | _(required)_ | Filename search string (case-insensitive substring match) |
