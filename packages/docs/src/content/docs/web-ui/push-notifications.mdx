...
PizzaPi can send browser push notifications when your agent finishes a task, hits an error, or needs your input — even if you've closed the tab.
...
|---|---|---|
| **Agent finished** (`agent_finished`) | Agent reaches the end of its task (`agent_end`) | The agent's final reply text when available; otherwise _"Your agent in \{session\} has finished its task."_ |
...
When the agent asks a single question with multiple-choice options, the push notification includes **action buttons** so you can reply directly — no need to open the browser. Up to two option buttons plus an inline text reply are shown. Quick reply is available on linked remote sessions by default.
...
When enabled, notifications from linked child sessions are silently skipped. The parent session's notifications still come through normally. This setting is per-subscription (per-device), so you can suppress on your phone but keep them on desktop. The toggle appears in the notifications dropdown on desktop, or in the mobile header menu on small screens. Suppression applies to browser Web Push subscriptions only — it is not currently enforced for native app push (ntfy).
...
If you change VAPID keys, all existing subscriptions become invalid. The server automatically cleans up stale subscriptions (410 Gone or 404 Not Found responses from push services), but users need to re-subscribe.
