...
Use plan mode to explore an unfamiliar codebase, plan multi-step changes, or gate the agent before it modifies anything.
...
When plan mode activates from the agent's `toggle_plan_mode` tool call, an in-conversation card confirms that read-only exploration is enabled. The `/plan` slash command instead updates the session toolbar and viewer status directly; no message is injected into the transcript. The session toolbar shows a **⏸ plan** pill so you can see at a glance that writes are blocked.
...
- **Approve** — `respond_to_trigger(triggerId, "approve", { action: "approve" })` — the child proceeds
- **Reject** — `respond_to_trigger(triggerId, "cancel", { action: "cancel" })` — the child stops
...
Pending plans are hidden from the transcript while the plan review panel is active in the composer area; once you respond, the styled card appears in the conversation.
...
Plan mode blocks writes through two layers. For the full technical details on how blocking works (tool restrictions, bash blocklist, sandbox overlay), see the [Plan Mode](/features/plan-mode/) feature doc.
