...
Toggle haptics from **Preferences → Notifications**, or with the vibration icon button in the toolbar. A quick test buzz fires when you enable it. The preference is saved in `localStorage` and persists across sessions. See [Preferences](/PizzaPi/web-ui/preferences/) for full details.
