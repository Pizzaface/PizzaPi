...
The **Web Terminal** is a full terminal emulator embedded in the PizzaPi browser UI. It gives you shell access on the runner machine — no SSH client or separate terminal app required.

## Opening the terminal

Toggle the terminal panel with the keyboard shortcut:

- **<kbd>Ctrl</kbd> + <kbd>`</kbd>** (backtick) — works on macOS, Linux, and Windows. PizzaPi intentionally uses Ctrl (not Cmd) on macOS to avoid conflicting with the system's <kbd>Cmd</kbd> + <kbd>`</kbd> window-switching shortcut.
...
1. When you open a terminal, the UI sends a `POST /api/runners/terminal` request to the relay server, which registers the terminal and returns its ID. The `new_terminal` command is forwarded to the runner on the browser's first `terminal_resize`, so the PTY is spawned with the correct initial dimensions (deferred spawn).
...
4. The runner auto-detects your default shell (`$SHELL`, or falls back to `/bin/bash` on macOS/Linux and PowerShell on Windows).
...
- **One PTY per tab.** Each tab is a single shell. When the shell exits (e.g., you type `exit`), the tab shows `[Process exited with code N]` and disconnects. Open a new tab to get a fresh shell.
