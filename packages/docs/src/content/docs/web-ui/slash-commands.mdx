...
| `/goal "{condition}" [flags]` | Set a success condition and optional budget (`--max-turns`, `--max-tokens`, `--max-cost`). The session stops when the goal is met or a budget runs out. |
