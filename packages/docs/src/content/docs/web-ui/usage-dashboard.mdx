...
The **Usage Dashboard** shows token consumption, costs, and session activity across all projects on a runner.
...
- **Cost by Project** — which projects cost the most. Note: both charts share the same session-sorted ordering; the membership is the top projects by cost, but the visual order is by session count.
...
For **Anthropic**, this shows your OAuth subscription rate-limit windows (5-hour, 7-day, Opus, Sonnet, co-work, and OAuth apps) with utilization bars and reset times. If extra usage is enabled, it also shows your monthly limit and credits used.
...
- **Runner-scoped data** — The dashboard shows sessions from the usage database on the machine that hosts the selected runner. Multiple runner processes on the same host share one `usage.db` and will return the same combined data.
...
- **Cost data depends on the provider** — Not all providers return per-token cost breakdowns. Sessions without cost data are excluded from session cost totals and averages; their events contribute 0 to the daily, model, and project cost charts.
