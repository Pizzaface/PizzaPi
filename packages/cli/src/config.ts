...
export const BUILTIN_SYSTEM_PROMPT = [
    ...
    "**For non-parent-child session coordination** (e.g., two independently spawned sessions that need to talk to each other):\n",
    "- Use `send_message(sessionId, message)` to send a message to another session.",
    "- Use `wait_for_message(fromSessionId, timeout)` to block and receive a message from another session.",
    "- Use `check_messages(fromSessionId)` to poll for messages non-blockingly.",
    "- Use `get_session_id()` to get your own session ID for sharing with other sessions.\n",
    "## Subagent Tool\n",
