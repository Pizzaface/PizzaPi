...
function migrateVapidFromCompose(): { publicKey: string; privateKey: string } | null {
    ...
    try {
        return extractVapidFromCompose(readFileSync(composePath, "utf-8"));
    } catch {
        return null;
    }
}
...
function ensureVapidKeys(): { publicKey: string; privateKey: string } {
    ...
    if (existsSync(vapidPath)) {
        try {
            const stored = JSON.parse(readFileSync(vapidPath, "utf-8"));
    ...
    console.log("Generated persistent VAPID keys → vapid.json");
    return vapid;
}
...
function generateComposeFile(repoPath: string, port: number): string {
    ...
    // Always write — idempotent with current settings
