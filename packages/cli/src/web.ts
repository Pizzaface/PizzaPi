...
export interface WebConfig {
    ...
    proxyDepth?: number;
    /** Host-based tunnel domain ([scheme://]host[:port], persisted). */
    tunnelDomain?: string;
...
export interface CaddyConfig {
    /** Wildcard host Caddy terminates TLS for, no scheme/port (e.g. "t.example.com"). */
    host: string;
    /** TLS port Caddy listens on. */
    port: number;
    /** DNS-01 provider module (e.g. "cloudflare"), or null to use Caddy's internal CA. */
...
/**
 * Decide whether to emit a Caddy service, and with what settings.
 *
 * Host-based tunnels need wildcard TLS for `*.<tunnelDomain>`, which neither
 * `tailscale serve` nor a plain reverse proxy can issue. Caddy can, two ways:
 *
 *   - No DNS credentials → `tls internal` (Caddy's own CA). Works on a private
 *     tailnet/LAN with no public reachability; trust the root once per device.
...
export function resolveCaddyConfig(
    ...
    if (!config.caddy) return { caddy: null };

    const raw = config.tunnelDomain?.trim();
    if (!raw) {
        return { caddy: null, error: "PIZZAPI_CADDY is set but PIZZAPI_TUNNEL_DOMAIN is not — nothing to terminate TLS for." };
    }

    // Mirrors the relay's parser in packages/server/src/routes/tunnel-host.ts.
    const rest = raw.replace(/^https?:\/\//, "").replace(/\/+$/, "");
    const colon = rest.indexOf(":");
    const host = (colon >= 0 ? rest.slice(0, colon) : rest).toLowerCase();
    const port = colon >= 0 ? parseInt(rest.slice(colon + 1), 10) : 443;
    if (!host || host.includes("/") || !Number.isInteger(port) || port < 1 || port > 65535) {
        return { caddy: null, error: `Cannot parse PIZZAPI_TUNNEL_DOMAIN (${raw}) as [scheme://]host[:port].` };
    }
    if (host === "localhost" || host.endsWith(".localhost")) {
        return { caddy: null, warning: `Skipping Caddy: *.localhost tunnels resolve to loopback and need no TLS.` };
    }
...
export function buildCaddyfile(c: CaddyConfig): string {
    const tls = c.dnsProvider
        ? `\ttls {\n\t\tdns ${c.dnsProvider} {env.PIZZAPI_CADDY_DNS_TOKEN}\n\t}`
        : "\ttls internal";
    return [
        "# Auto-generated by `pizza web` — regenerated on each run.",
        "# Wildcard TLS for host-based tunnels; the relay routes on the Host header.",
        "{",
        "\tadmin off",
        "}",
        "",
...
export function generateComposeFile(opts: {
    repoPath: string;
    config: WebConfig;
    useDevUi: boolean;
    ...
    // PIZZAPI_TUNNEL_DOMAIN: env var overrides config, and gets persisted back.
    const envTunnelDomain = process.env.PIZZAPI_TUNNEL_DOMAIN?.trim();
    if (envTunnelDomain !== undefined) {
        config.tunnelDomain = envTunnelDomain || undefined;
    }

    // PIZZAPI_CADDY: env var overrides config, and gets persisted back.
    if (process.env.PIZZAPI_CADDY !== undefined) {
    ...
    const tunnelDomainLine = config.tunnelDomain
        ? `      - PIZZAPI_TUNNEL_DOMAIN=${config.tunnelDomain}\n`
        : `      # - PIZZAPI_TUNNEL_DOMAIN=\n`;
    ...
    if (caddyWarning) log.warn(caddyWarning);
    let caddyBlock = "";
    let caddyVolumes = "";
    if (caddy) {
        ...
        if (config.trustProxy !== true) {
            log.warn(
                "Caddy fronts the relay but PIZZAPI_TRUST_PROXY is not true — client IPs will be Caddy's. " +
                "Set PIZZAPI_TRUST_PROXY=true PIZZAPI_PROXY_DEPTH=1 once the relay port is no longer directly reachable.",
            );
        }
        log.info(`Caddy: *.${caddy.host}:${caddy.port} → ${caddy.upstream}:7492 (${caddy.dnsProvider ? `DNS-01 via ${caddy.dnsProvider}` : "internal CA"})`);
    }
