...
describe("E2E: key rotation", () => {
    ...
    test("old API key is invalidated after re-register", async () => {
        ...
        const res1 = await req("POST", "/api/register", rotUser, { "x-real-ip": "10.0.1.1" });
        ...
        const res2 = await req("POST", "/api/register", rotUser, { "x-real-ip": "10.0.1.2" });
...
describe("E2E: change-password enforces password policy", () => {
    ...
    beforeAll(async () => {
        // Register user
        const regRes = await req("POST", "/api/register", cpUser, { "x-real-ip": "10.0.4.1" });
