...
[ -d ".project" ] && echo "  ✅ .project directory exists" || { echo "  ❌ .project directory missing"; ((errors++)); }
...
orphaned=$(find .project -name "[0-9]*.md" -not -path ".project/epics/*/*" 2>/dev/null | wc -l)
...
for file in $(find .project -name "*.md" -path "*/epics/*" -o -path "*/prds/*" 2>/dev/null); do
  if ! grep -q "^---" "$file"; then
